...
extern err_t noeud_etiquette_ecrire( noeud_t * noeud , void * etiquette , err_t (*affecter)( void * e1 , void * e2 ) ){
  if(noeud_existe(noeud)) affecter(noeud->etiquette,etiquette);
  return(OK) ;
}
...
extern booleen_t noeud_numero_rechercher( noeud_t ** result ,        /* Resultat: @ du noeud trouve */
				   ...
				   const int numero       )  /* Numero a rechercher dans l'arbre */
{
  if(noeud_existe(racine)){
    ...
    else if((noeud_numero_rechercher(result,racine->droit,numero))) return (VRAI);
    else return (noeud_numero_rechercher(result,racine->gauche,numero));
  }
  return(FAUX);
}
